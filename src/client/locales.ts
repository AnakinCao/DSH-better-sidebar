...
export const zh = {
  ...
  editorExplorerDesc: '开启后编辑器标签页变为「文件预览 + 可开关的文件树面板」：头部是路径输入框，树面板支持全局文件名搜索；新会话默认打开空文件预览页（树面板展开）。关闭后恢复纯编辑器',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  editorExplorerDesc: 'When on, editor tabs become "file preview + toggleable file-tree panel": the header is a path input and the panel offers a global file-name search; new conversations seed an empty preview tab with the tree panel open. Off restores the plain editor',

...
| `terminal` | 40 | 否 | 否 | 终端（nextTerminal 自增）。v0.17.0+ 支持右键「固定到工作区 / 固定到全局」：固定后切换会话不消失，在面板顶部 PinnedRail 跨会话呈现（global 任意会话可见；workspace 仅同 cwd 会话可见）；Agent 终端（`agent:<uuid>`）被 reconcile 移除时豁免保留为「已断开」（xterm 重连失败 banner 承担断开信号）；pin 元数据 `tab.pin = { scope, homeCwd? }` 随宿主会话 state 持久化，渲染期跨会话解析（`collectPinnedTabs`），不写跨会话投影 |
